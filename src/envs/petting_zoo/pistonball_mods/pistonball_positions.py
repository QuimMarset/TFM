...
class raw_env(AECEnv, EzPickle):
    ...
    def observe(self, agent):
        ...
        position_y = self.screen_height - position.y - self.wall_width
        ball_flags = self.compute_ball_position_wrt_piston(index, position)
        return np.array([position.x, position_y, *ball_flags])
    ...
    def state(self):
        ...
        for i in range(self.n_pistons):
            position = self.pistonList[i].position
            positions.append([position.x, position.y])
        ball_position = self.ball.position
        positions.append([ball_position[0], ball_position[1]])
