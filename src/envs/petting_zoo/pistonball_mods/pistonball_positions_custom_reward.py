...
class raw_env(AECEnv, EzPickle):
    ...
    def compute_ball_position_wrt_piston(self, index, position):
        x_start = position[0]
        x_end = x_start + self.piston_width

        x_high = self.wall_width + self.piston_width * (index + 2)
        x_low = self.wall_width + self.piston_width * (index - 1)

        ball_x = self.ball.position[0]
        ball_start = ball_x - self.ball_radius
        ball_end = ball_x + self.ball_radius

        if x_start <= ball_x <= x_end:
            # The ball is above the piston
            return [1, 0, 0]

        elif x_end <= ball_start <= x_high or x_end <= ball_x <= x_high:
            # The ball is at the right, but still visible
            return [0, 0, 1]

        elif x_low <= ball_end <= x_start or x_low <= ball_x <= x_start:
            # The ball is at the left, but still visible
            return [0, 1, 0]

        # The ball is not visible from the piston
        return [0, 0, 0]
    ...
    def observe(self, agent):
        ...
        ball_flags = self.compute_ball_position_wrt_piston(index, position)
    ...
    def get_current_frame(self):
        ...
        copy_frame = np.rot90(copy_frame, k=3)
        copy_frame = np.fliplr(copy_frame)
        del frame
        return copy_frame
    ...
    def state(self):
        ...
        positions.append([ball_position_x, ball_position_y])
    ...
    def compute_reward(self, current_ball_x_position, is_final_ball_x_position):
        final_reward = 100 if is_final_ball_x_position else 0 
        local_reward = self.alpha * (self.lastX - current_ball_x_position)
        reward = final_reward + local_reward
        rewards = [0] * (self.n_pistons - 1)
