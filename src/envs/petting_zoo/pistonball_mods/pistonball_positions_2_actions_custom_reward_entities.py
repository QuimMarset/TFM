...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(
        ...
        self.ball_flgas = np.zeros(self.n_pistons) # ball above the piston
            ...
            zip(
                ...
                [
                    gymnasium.spaces.Box(
                        low=0,
                        high=1,
                        shape=(3, ), # ball flags: the ball is on top, left or right
                        dtype=np.float32,
                    )
                ]
                * self.n_pistons,
        ...
        if state_entity_mode:
            ...
            self.state_space = gymnasium.spaces.Box(
                low=0,
                high=max(1, self.screen_width, self.screen_height),
                shape=(2*(self.n_pistons),), # ball flag and x position
                dtype=np.float32,
            )
        else:
            ...
            self.state_space = gymnasium.spaces.Box(
                low=0,
                high=1,
                shape=(3*self.n_pistons,), # the ball flags for all the pistons
                dtype=np.float32,
            )
    ...
    def state_entity_mode(self):
        """
        State structure (third feature is IS_AGENT):
        [(p.x, *ball_flags) for p in pistons]
        """
        state_matrix = np.zeros((self.n_pistons, 2))
        for i, piston in enumerate(self.pistonList):
            state_matrix[i, 0] = self.process_position(piston.position)[0]
            state_matrix[i, 1] = self.ball_flags[i]
