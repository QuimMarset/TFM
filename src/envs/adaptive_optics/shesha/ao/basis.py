...
from envs.adaptive_optics.shesha.sutra_wrap import Dms, Rtc_FFF as Rtc

import envs.adaptive_optics.shesha.config as conf
...
def compute_cmat_with_Ptt_modified(rtc: Rtc, Ptt: np.ndarray, nfilt: int):
    """
    Computes a command matrix on the Ptt basis and load it in the GPU
    In this version we consider that we have remove first two modes and added TT already
    """
    D = np.array(rtc.d_control[0].d_imat)
    # D = ao.imat_geom(wfs,config.p_wfss,config.p_controllers[0],dms,config.p_dms,meth=0)
    Ptt_filt = np.zeros((Ptt.shape[0], Ptt.shape[1] - nfilt))
    # Add Ptt and filter, Ptt does not have TT commands
    Ptt_filt[:, :Ptt_filt.shape[1] - 2] = Ptt[:, :Ptt.shape[1] - (nfilt + 2)]
    Ptt_filt[:, Ptt_filt.shape[1] - 2:] = Ptt[:, Ptt.shape[1] - 2:]
    ...
    Dmp = np.linalg.inv(Dm.T.dot(Dm)).dot(Dm.T)
    # Command matrix
    cmat = Ptt_filt.dot(Dmp)
    rtc.d_control[0].set_cmat(cmat.astype(np.float32))

    return cmat.astype(np.float32)

def compute_cmat_with_Ptt(rtc: Rtc, Ptt: np.ndarray, Btt: np.ndarray, nfilt: int, filter_first_two_modes: False):
    """ Compute a command matrix on the Ptt basis and load it in the GPU

    :parameters:

        rtc: (Rtc): rtc object

        Ptt: (np.ndarray[ndim=2, dtype=np.float32]) : Ptt to volts matrix

        nfilt: (int): number of modes to filter
    """
    D = np.array(rtc.d_control[0].d_imat)
    # D = ao.imat_geom(wfs,config.p_wfss,config.p_controllers[0],dms,config.p_dms,meth=0)
    if filter_first_two_modes:
        Ptt_filt = np.zeros((Ptt.shape[0] + 2, Ptt.shape[1] - nfilt))
        # Add Ptt and filter, Ptt does not have TT commands
        Ptt_filt[:-2, :Ptt_filt.shape[1] - 2] = Ptt[:, 2:Ptt.shape[1] - nfilt]
        # Add Tip Tilt with Btt
        Ptt_filt[:, Ptt_filt.shape[1] - 2:] = Btt[:, Btt.shape[1] - 2:]
    else:
        Ptt_filt = np.zeros((Ptt.shape[0] + 2, Ptt.shape[1] - nfilt + 2))
        # Add Ptt and filter, Ptt does not have TT commands
        Ptt_filt[:-2, :Ptt_filt.shape[1] - 2] = Ptt[:, :Ptt.shape[1] - nfilt]
        # Add Tip Tilt with Btt
        Ptt_filt[:, Ptt_filt.shape[1] - 2:] = Btt[:, Btt.shape[1] - 2:]
