...
from envs.adaptive_optics.shesha.constants import CONST

from envs.adaptive_optics.shesha.ao import imats, cmats, tomo, basis, modopti

from envs.adaptive_optics.shesha.util import utilities, rtc_util
from envs.adaptive_optics.shesha.init import dm_init
from typing import List

import numpy as np
from envs.adaptive_optics.shesha.sutra_wrap import (carmaWrap_context, Sensors, Dms, Target, Rtc_brahma,
                               Rtc_cacao_FFF, Atmos, Telescope)
from envs.adaptive_optics.shesha.sutra_wrap import Rtc_FFF as Rtc
...
def rtc_standalone(context: carmaWrap_context, nwfs: int, nvalid: list, nactu: int,
    ...
    if brahma:
        ...
        if fp16:
            from envs.adaptive_optics.shesha.sutra_wrap import Rtc_cacao_FHF
            rtc = Rtc_cacao_FHF("compass_calPix", "compass_loopData")
        else:
            rtc = Rtc_cacao_FFF("compass_calPix", "compass_loopData")
    else:
        if fp16:
            from envs.adaptive_optics.shesha.sutra_wrap import Rtc_FHF
            rtc = Rtc_FHF()
        else:
            rtc = Rtc()
    for k in range(nwfs):
