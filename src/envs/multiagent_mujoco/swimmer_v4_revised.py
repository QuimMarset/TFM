...
class SwimmerEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.state_entity_mode:
            self._get_obs = self.get_state_entity
        else:
            self._get_obs = self.get_state_default
        ...
        MujocoEnv.__init__(
            self, "swimmer.xml", 4, observation_space=observation_space, 
    ...
    def _set_entity_attributes(self):
        ...
        # The two joints plus the extra 3 joints between the tip and the floow
        self.n_entities_state = 5
        # Both obs and state features are pos and vel
    ...
    def get_state_entity(self):
        """
        Defined as the composition of the joint entities removing duplicates
        Thus, the position and velocity of the front tip and both rotors (i.e. hinge joints in the xml)
        """
        position = self.data.qpos.flat.copy()
        velocity = self.data.qvel.flat.copy()
        state = []
        for i in range(-5, 0):
            state.append(position[i])
            state.append(velocity[i])
