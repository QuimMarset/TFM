...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, emb, heads, mask, ff_hidden_mult=4, dropout=0.0, save_memory=False):
        super().__init__()
    ...
    def forward(self, q_k_mask):
        q, k, mask = q_k_mask
        # Use checkpointing for the attention module to save memory
        if self.save_memory:
            attended = checkpoint(self.attention, q, k, mask)
        else:
...
class Transformer(nn.Module):
    ...
    def __init__(self, emb, heads, depth, ff_hidden_mult=4, dropout=0.0, save_memory=False):
        super().__init__()
        # transformer blocks
        tblocks = []
        for _ in range(depth):
            tblocks.append(
                TransformerBlock(emb=emb, heads=heads, mask=False, 
                                 ...
                                 dropout=dropout, save_memory=save_memory)
            )
